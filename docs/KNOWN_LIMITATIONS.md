...
**這是開放公開註冊前唯一的阻擋項。** 其餘功能在缺少寄信服務時都能運作，
但使用者一旦忘記密碼就沒有任何自助途徑——只有能讀伺服器日誌的人才救得回來。
在補上寄信服務之前，若要對外開放，應同時提供一個可聯絡的求助管道。

### 決策狀態

**2026-09-09：寄信服務尚未選定，暫緩。**
候選為 SMTP（自備信箱）或 Resend、SendGrid 之類的 API 服務。
選擇會牽涉外部帳號申請與網域驗證（寄件網域需設定 SPF／DKIM，
否則信件多半會進垃圾信匣），因此留待實際要開放註冊時再定。
...
5. 設定寄件網域的 SPF／DKIM，並實際寄一封確認不落入垃圾信匣
